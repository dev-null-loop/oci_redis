variable "backup_id" {
  description = "The ID of the Oracle Cloud Infrastructure Cache Backup from which this cluster was created.Mutually exclusive with 'importFromObjectStorageDetails'."
  type        = string
  default     = null
}
...
variable "compartment_id" {
  description = "The [OCID](https://docs.cloud.oracle.com/iaas/Content/General/Concepts/identifiers.htm#Oracle) of the compartment that contains the cluster."
  type        = string
}
...
variable "security_attributes" {
  description = "Security attributes for redis cluster resource. Each key is predefined and scoped to a namespace. Example: `{\"Oracle-ZPR\": {\"MaxEgressCount\": {\"value\": \"42\", \"mode\": \"enforce\"}}}`"
  type        = map(string)
  default     = null
}

variable "subnet_id" {
  description = "The [OCID](https://docs.cloud.oracle.com/iaas/Content/General/Concepts/identifiers.htm#Oracle) of the cluster's subnet."
  type        = string
}
